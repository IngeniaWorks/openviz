...
- [ ] T026 Migrate Media and Create-new submenus in `src/components/workbench/WorkbenchToolbar.tsx` to Radix `DropdownMenu` (`@radix-ui/react-dropdown-menu`): keyboard navigation, Escape-to-close, focus return; keep C-1.4/C-1.5 behavior tests green (Constitution IV, research R5)
- [ ] T027 Split `src/store/slices/workbenchSlice.ts` (909 lines) into focused modules under `src/store/slices/` (e.g. tool slice + existing concerns) composed into the same store; every file ≤300 lines; all tests green after extraction (Constitution V, research R6)
- [ ] T028 Split `src/components/workbench/workbench.tsx` (636 lines): extract media-upload handlers to `src/components/workbench/hooks/useWorkbenchMediaUpload.ts` and other handler clusters into existing/new `useWorkbench*` hooks; view ≤300 lines; tests green (Constitution V, research R6)
- [ ] T029 Run full acceptance matrix from `quickstart.md` (17 manual rows + automated gate): `pnpm lint && pnpm typecheck && pnpm test:ci`; record new coverage numbers and ratchet the floor in `vitest.config.ts` if any metric improved ≥1 point
- [ ] T030 Convergence check: `git diff --stat main` scoped to this feature; confirm no changes outside workbench toolbar scope except constitution-mandated refactors; update `spec-kit-tdd-plan.md` Sprint 3 status
